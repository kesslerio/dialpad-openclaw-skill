...
25. **Send SMS managed environment & direct API fallback:** `bin/send_sms.py` executes generated Dialpad CLI calls through a managed environment built into the skill copy: the wrapper's own interpreter runs `generated/dialpad` with `vendor/` (constructed from the pinned, hash-verified `requirements.txt` by `scripts/build_vendor.py`; provenance and delivery in `docs/reference/vendor-build.md` and `docs/reference/runtime-copies.md`) on `PYTHONPATH`, so click always imports from the repo tree — never from an ambient system python, a `PATH` lookup, or a `uv` binary that non-interactive runtimes (systemd, agent exec, cron) do not reliably carry (the #89 `uv`-discovery fix regressed in #155 exactly this way). If the generated CLI or its Python runtime dependencies are missing anyway, `bin/send_sms.py` falls back to direct Dialpad API execution as a safety net, and a draft claimed for such a local failure returns to a retryable `pending`/`risk_pending` state instead of being terminally `failed`, so approved outbound SMS messages are neither lost nor stuck on environment gaps.
