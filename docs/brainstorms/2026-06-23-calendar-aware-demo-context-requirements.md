...
Missed-call enrichment should tell the operator whether a prospect already has a scheduled demo, especially when CRM shows a demo-request deal. The follow-up should turn calendar context from a silent configuration gap into an explicit source that can produce warmer, more accurate approval drafts.
...
- AE1. **Covers R1, R2, R4, R6.** Given a missed call from a demo-request prospect with no scheduled demo found, the card shows CRM usable and calendar not found, and the draft offers the booking link plus help coordinating.
