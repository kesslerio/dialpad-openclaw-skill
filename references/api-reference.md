...
- `bin/list_calls.py` provides agent-safe recent call history with `--hours` or `--today`, optional missed-call filtering, `--local` for the local calls SQLite store, `--live` for an explicit Dialpad query, and `--json` for a machine-readable envelope. When `DIALPAD_LOG_URL` is set, shared history is the default and `--local-log` names that mode explicitly. JSON summaries include `contact_phone` when Dialpad exposes the caller number.
...
- `bin/list_sms_thread.py` provides read-only SMS history for one phone number. When `DIALPAD_LOG_URL` is set it reads the authenticated shared log; otherwise it reads the local SQLite path. Use it before claiming that a missed-call contact has no visible SMS response. Set `DIALPAD_SMS_DB` to override the local SQLite path, for example `/home/art/niemand/logs/sms.db` in AlphaClaw runtimes.
- `bin/list_sms_inbox.py` provides read-only inbound SMS history across all threads, using the shared log when `DIALPAD_LOG_URL` is configured and supporting `--unread-only`.
