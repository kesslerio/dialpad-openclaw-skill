...
- `bin/get_call_transcript.py` retrieves transcript text for one Dialpad call through the supported agent wrapper surface. It returns explicit unavailable results when Dialpad has no transcript instead of treating missing transcript text as a successful transcript.
