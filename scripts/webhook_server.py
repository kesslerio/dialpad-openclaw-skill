...
DIALPAD_PUBLIC_PROSPECT_SEARCH_CACHE_TTL_SECONDS = int(
    os.environ.get(
        "DIALPAD_PUBLIC_PROSPECT_SEARCH_CACHE_TTL_SECONDS",
        str(getattr(phone_intelligence, "DEFAULT_PUBLIC_SEARCH_TTL_SECONDS", 6 * 60 * 60)),
    )
)
...
MERGED_FLOW_FALLBACK_WARNING_THRESHOLD = 3
_MERGED_FLOW_COUNTERS = {
    "callback": 0,
    "fallback": 0,
...
def insert_pending_draft(job_id, event_dict, fallback_draft, callback_token,
    ...
    try:
        ...
        try:
            ...
            conn.close()
    except Exception as exc:  # noqa: BLE001 - merged flow degrades to local-only delivery.
    ...
    try:
        ...
        try:
            ...
            if cursor.rowcount == 0:
                ...
                return None
            row = conn.execute(
                f"SELECT job_id, created_at_ms, event_json, fallback_draft, callback_token, "
            ...
            return {
                ...
                "fallback_draft": row[3],
                "callback_token": row[4],
                "route_chat_id": row[5],
                "route_thread_id": row[6],
            }
        finally:
            conn.close()
...
def _run_context_command(command, query):
    """Run an optional local context command and return parsed JSON or plain text."""
    ...
    if not args:
        return {"usable": False, "status": "disabled"}
    args.append(str(query or "").strip())
    ...
    try:
        completed = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=DIALPAD_CONTEXT_LOOKUP_TIMEOUT_SECONDS,
        )
    except FileNotFoundError:
        return {"usable": False, "status": "unavailable"}
    except subprocess.TimeoutExpired:
        return {"usable": False, "status": "timeout"}
    except Exception as exc:  # noqa: BLE001 - context lookup must not break webhook processing.
        print(f"⚠️  Sales context lookup failed ({type(exc).__name__})")
...
def _context_name(sender_enrichment, normalized_event):
    ...
    return (
        sender_enrichment.get("contact_name")
        or inbound_context.get("contactName")
        or first_contact.get("contactName")
        or normalized_event.get("sender")
    )
...
def meeting_logistics_intent(text):
    """Return True for bounded meeting/timing messages that warrant calendar lookup."""
    body = str(text or "").strip().lower()
    if not body:
        return False
    patterns = (
        r"\brunning\s+\d+\s*(?:min|mins|minutes?)?\s*late\b",
        r"\b(?:running|be|i'?m|im)\s+late\b",
        r"\b(?:on my way|joining now|join(?:ing)?\s+(?:now|shortly)|dial(?:ing)? in)\b",
        r"\b(?:reschedule|can'?t make|cannot make|won'?t make|miss(?:ing)?\s+(?:the\s+)?(?:call|meeting|demo))\b",
        r"\b(?:zoom|meet|meeting|demo)\s+(?:link|url)\b",
        r"\b(?:where|what)\s+(?:is|'?s)\s+(?:the\s+)?(?:zoom|meet|meeting|demo)\s+(?:link|url)\b",
    )
...
def _canonical_e164_phone(value):
    if phone_intelligence is not None:
        normalized = phone_intelligence.normalize_phone(value)
        if normalized:
            return normalized
...
def _normalize_public_prospect_result(result, inputs):
    normalized_phone = _canonical_e164_phone(inputs.get("phone"))
    ...
    for entry in raw_evidence:
        item = _sanitize_public_evidence(entry, normalized_phone)
        if item:
    ...
    qualified_evidence = [
        ...
        if (item.get("phoneCorroboration") or {}).get("matched") and _evidence_business_like(item)
    ]
...
def _public_prospect_cache_key(inputs):
    return json.dumps(inputs or {}, sort_keys=True, separators=(",", ":"))


def _public_prospect_cache_policy(command):
    return f"public-prospect-v1|{command}"


def _run_json_command(args, *, input_text=None, timeout=4, warning_label="command", invalid_json_status="invalid_payload"):
    try:
        completed = subprocess.run(
            args,
            input=input_text,
    ...
    except Exception as exc:  # noqa: BLE001 - caller commands must fail closed.
        print(f"WARNING: {warning_label} failed ({type(exc).__name__})")
        return {"ok": False, "status": "request_failed"}
    if completed.returncode != 0:
        return {"ok": False, "status": f"exit_{completed.returncode}"}
    stdout = getattr(completed, "stdout", "") or ""
    stderr = getattr(completed, "stderr", "") or ""
    if len(stdout) > DIALPAD_JSON_COMMAND_MAX_OUTPUT_CHARS or len(stderr) > DIALPAD_JSON_COMMAND_MAX_OUTPUT_CHARS:
        return {"ok": False, "status": "output_too_large"}
    try:
        payload = json.loads(stdout or "{}")
    except json.JSONDecodeError:
        return {"ok": False, "status": invalid_json_status}
...
def lookup_public_prospect_context(caller_intelligence):
    ...
    cache_key = _public_prospect_cache_key(inputs)
    cache_policy = _public_prospect_cache_policy(command)
    if phone_intelligence is not None:
        cached = phone_intelligence.cache_json_get("public_prospect", cache_key, cache_policy)
    ...
    command_result = _run_json_command(
        args,
        input_text=json.dumps(inputs, separators=(",", ":")),
        timeout=DIALPAD_PUBLIC_PROSPECT_SEARCH_TIMEOUT_SECONDS,
        warning_label="Public prospect search",
        invalid_json_status="unsafe_output",
    )
    if not command_result.get("ok"):
        return {"usable": False, "status": command_result.get("status") or "unavailable"}
    result = command_result.get("payload") or {}
    normalized = _normalize_public_prospect_result(result, inputs)
    if phone_intelligence is not None:
        phone_intelligence.cache_json_set(
            "public_prospect",
            cache_key,
            cache_policy,
            normalized,
            ttl_seconds=DIALPAD_PUBLIC_PROSPECT_SEARCH_CACHE_TTL_SECONDS,
        )
    return normalized
...
def _evidence_mentions_reverse_name(evidence, reverse_name):
    required = {
        token
        for token in re.findall(r"[a-z0-9]+", str(reverse_name or "").lower())
        if len(token) > 1
    }
    if not required:
        return False
    ...
    available = set(re.findall(r"[a-z0-9]+", haystack))
...
def _qualifying_phone_corroborated_business_evidence(caller_context):
    ...
    for evidence in public_context.get("evidence") or []:
        if (
            ...
            and _evidence_mentions_reverse_name(evidence, reverse_name)
        ):
            return evidence
    return None
...
def sync_dialpad_contact_from_enrichment(normalized_event, sender_enrichment=None):
    ...
    inbound_context = normalized_event.get("inbound_context") or {}
    phone = _canonical_e164_phone((caller_context.get("phone") or {}).get("e164")) or _canonical_e164_phone(normalized_event.get("sender_number"))
    if not phone:
...
def prepare_inbound_reply_event(normalized_event, sender_enrichment=None, line_display=None, recent_context=None):
...
def lookup_sales_calendar_context(normalized_event, crm_context=None, sender_enrichment=None):
    ...
    result = _run_context_command(DIALPAD_CALENDAR_CONTEXT_COMMAND, query)
    if result.get("usable") is not True:
        return {"usable": False, "status": result.get("status") or "unavailable"}
...
def _meeting_reply_message(normalized_event, sender_enrichment, _crm_context, _calendar_context):
    ...
    if re.search(r"\breschedule|can'?t make|cannot make|won'?t make\b", body):
        return f"Hi {greeting}, no worries. Send over a few times that work and we'll find a new slot."
...
def _render_merged_card(job_id, draft_text, claimed_row, path, elapsed_ms=0):
    ...
    provenance = _build_draft_provenance(event)
    if provenance:
...
class DialpadWebhookHandler(BaseHTTPRequestHandler):
        ...
        if direction == "inbound":
            ...
            if (
                ...
                if (
                    ...
                    _merged_callback_url = DIALPAD_DRAFT_CALLBACK_URL
                    _merged_event = {
                        "event_type": "sms",
                        ...
                        "crm_context": normalized_sms.get("crm_context"),
                        "calendar_context": normalized_sms.get("calendar_context"),
                        "comms_context": normalized_sms.get("comms_context"),
                        "rich_reply": normalized_sms.get("rich_reply"),
                    ...
                    _merged_pending_inserted = insert_pending_draft(
    ...
    def handle_draft_callback(self):
        ...
        if claimed is None:
            _record_merged_flow_path("callback_lost")
            print(f"[merged-flow] job_id={job_id} path=callback_lost elapsed_ms=N/A "
                  f"draft_chars={len(draft)} {_merged_flow_counter_suffix()}")
